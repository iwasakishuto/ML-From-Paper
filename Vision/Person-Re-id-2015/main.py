...
#=== モデル定義 ===
def generate_model(weight_decay=cfg.TRAIN.WEIGHT_DECAY, print_summary=True):
    """
    ...
    x1_input = keras.layers.Input(shape=(*cfg.DATA.ARRAY_SIZE, 3), name="x1_input")
    x2_input = keras.layers.Input(shape=(*cfg.DATA.ARRAY_SIZE, 3), name="x2_input")

    #=== Tied Convolution.1 with max pooling ===
    share_conv_1 = keras.layers.Conv2D(20, 5, activation="relu", kernel_regularizer=l2(weight_decay), name="share_conv_1")
    ...
    #=== Tied Convolution.2 with max pooling ===
    share_conv_2 = keras.layers.Conv2D(25, 5, activation="relu", kernel_regularizer=l2(weight_decay), name="share_conv_2")
    x1 = share_conv_2(x1)
    x2 = share_conv_2(x2)
    x1 = keras.layers.MaxPooling2D(pool_size=(2, 2))(x1)
    x2 = keras.layers.MaxPooling2D(pool_size=(2, 2))(x2)
    ...
    conv_3_1 = keras.layers.Conv2D(25, 5, strides=(5, 5), activation="relu", kernel_regularizer=l2(weight_decay), name="conv_3_1")
    conv_3_2 = keras.layers.Conv2D(25, 5, strides=(5, 5), activation="relu", kernel_regularizer=l2(weight_decay), name="conv_3_2")
    x1 = conv_3_1(x1)
    x2 = conv_3_2(x2)

    #=== Across-Patch Features ===
    conv_4_1 = keras.layers.Conv2D(25, 3, activation="relu", kernel_regularizer=l2(weight_decay), name="conv_4_1")
    conv_4_2 = keras.layers.Conv2D(25, 3, activation="relu", kernel_regularizer=l2(weight_decay), name="conv_4_2")
    x1 = conv_4_1(x1)
    x2 = conv_4_2(x2)
    ...
    y = keras.layers.Dense(500, kernel_regularizer=l2(weight_decay), activation='relu')(y)
    y = keras.layers.Dense(2,   kernel_regularizer=l2(weight_decay), activation='softmax')(y)
...
def _generate_negative_pair(mode='train'):
    with h5py.File(cfg.DATA.CREATED_FILE, 'r') as f:
        ...
        image_x = _image_augmentation(f[str(i)][x])
        image_y = _image_augmentation(f[str(j)][y])
        return image_x, image_y
...
#=== GradCAM のモデル　===
def generate_model_forGC(weight_decay=cfg.TRAIN.WEIGHT_DECAY, print_summary=True):
    """
    関数の概要：GradCAM 用にモデルを書き換える。
    @param  weight_decay：正則化に重み減衰を行うときのscale
    @return model       ：compile 済みのモデル。モデルの構成は、論文の通り。
    kernel_regularizer を削除する。
    """
    #=== Input Pair of images ===
    x1_input = keras.layers.Input(shape=(*cfg.DATA.ARRAY_SIZE, 3), name="x1_input")
    x2_input = keras.layers.Input(shape=(*cfg.DATA.ARRAY_SIZE, 3), name="x2_input")

    #=== Tied Convolution.1 with max pooling (Divide separately ) ===
    x1_conv_1 = keras.layers.Conv2D(20, 5, activation="relu", name="x1_conv_1")
    x2_conv_1 = keras.layers.Conv2D(20, 5, activation="relu", name="x2_conv_1")
    x1 = x1_conv_1(x1_input)
    x2 = x2_conv_1(x2_input)
    x1 = keras.layers.MaxPooling2D(pool_size=(2, 2))(x1)
    x2 = keras.layers.MaxPooling2D(pool_size=(2, 2))(x2)

    #=== Tied Convolution.2 with max pooling (Divide separately ) ===
    x1_conv_2 = keras.layers.Conv2D(25, 5, activation="relu", name="x1_conv_2")
    x2_conv_2 = keras.layers.Conv2D(25, 5, activation="relu", name="x2_conv_2")
    x1 = x1_conv_2(x1)
    x2 = x2_conv_2(x2)
    x1 = keras.layers.MaxPooling2D(pool_size=(2, 2))(x1)
    x2 = keras.layers.MaxPooling2D(pool_size=(2, 2))(x2)
    ...
    #=== Patch Summary Features ===
    conv_3_1 = keras.layers.Conv2D(25, 5, strides=(5, 5), activation="relu", name="conv_3_1")
    conv_3_2 = keras.layers.Conv2D(25, 5, strides=(5, 5), activation="relu", name="conv_3_2")
    x1 = conv_3_1(x1)
    x2 = conv_3_2(x2)

    #=== Across-Patch Features ===
    conv_4_1 = keras.layers.Conv2D(25, 3, activation="relu", name="conv_4_1")
    conv_4_2 = keras.layers.Conv2D(25, 3, activation="relu", name="conv_4_2")
    x1 = conv_4_1(x1)
    x2 = conv_4_2(x2)
    x1 = keras.layers.MaxPooling2D(pool_size=(2, 2), padding='same')(x1)
    x2 = keras.layers.MaxPooling2D(pool_size=(2, 2), padding='same')(x2)
...
if __name__ == '__main__':
    ...
    model.load_weights('checkpoint0303_114.weight')
    ...
    model.save_weights('systemic.weight')
    print("saved weight to systemic.weights")
